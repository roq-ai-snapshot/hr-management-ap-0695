...
import { useRoqClient, useApplicantFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ApplicantInterface, unknown>;
...
export function ApplicantListPage(props: ApplicantListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['resume.contains', 'status.contains'],
          ...params.filters,
        },
        'applicant',
      ),
  ...
  const { data, error, isLoading, mutate } = useApplicantFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.applicant.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ApplicantInterface) => {
    if (hasAccess('applicant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/applicants/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    { id: 'resume', header: 'Resume', accessorKey: 'resume' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.application_date ? format(record?.application_date, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/applicants/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('applicant', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/applicants/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('applicant', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="applicant" />
        </Flex>

        {hasAccess('applicant', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/applicants/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
