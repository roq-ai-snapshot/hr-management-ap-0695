...
import { useApplicantCount, useCustomerCount, useHrManagerCount } from 'lib/roq';

import { ApplicantListPage as ShowCaseList } from 'pages/applicants';

function DashboardPage() {
  const { session } = useSession();

  const applicantStat = useApplicantCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Applicants',
      result: applicantStat?.data,
      isLoading: applicantStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`applicants`} passHref legacyBehavior>
                  <Button
